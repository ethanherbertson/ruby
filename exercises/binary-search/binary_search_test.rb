...
class BinarySearchTest < Minitest::Test
  ...
  def test_it_raises_error_for_data_not_in_list
    ...
    assert_raises RuntimeError do
      BinarySearch.new([1, 3, 6]).search_for(0)
    end
    assert_raises RuntimeError do
      BinarySearch.new([1, 3, 6]).search_for(4)
    end
    assert_raises RuntimeError do
      BinarySearch.new([1, 3, 6]).search_for(7)
    end
    assert_raises RuntimeError do
      BinarySearch.new([1, 3]).search_for(2)
    end
    assert_raises RuntimeError do
      BinarySearch.new([1, 3]).search_for(0)
    end
    assert_raises RuntimeError do
      BinarySearch.new([1, 3]).search_for(4)
    end
  end
  ...
  def test_it_finds_correct_position_in_a_huge_list
    # skip
    binary = BinarySearch.new((1..64000).to_a)
    assert_equal 20, binary.search_for(21)
    assert_equal 33, binary.search_for(34)
    assert_equal 21509, binary.search_for(21510)
  end
end
